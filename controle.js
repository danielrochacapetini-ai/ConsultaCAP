...
function registrarItem() {
    ...
    var tipo = document.getElementById('tipo').value; // CAMPO TIPO
...
function renderizarLista() {
    ...
    registrosFiltrados.forEach(function(r) {
        ...
        html += '<div class="registro-campo"><span class="label">Tipo</span><span class="valor">' + tipoValor + '</span></div>'; // CAMPO TIPO
...
function abrirEdicao(id) {
    ...
    modalHtml += '<div class="form-group"><label>TIPO</label><select id="editTipo"><option value="FABRICADO"' + (registro.tipo === 'FABRICADO' ? ' selected' : '') + '>FABRICADO</option><option value="COMPRADO"' + (registro.tipo === 'COMPRADO' ? ' selected' : '') + '>COMPRADO</option></select></div>'; // CAMPO TIPO
...
function salvarEdicao(id) {
    ...
    var tipo = document.getElementById('editTipo').value; // CAMPO TIPO
...
function exportarPDF() {
    ...
    var colunas = ['CÓDIGO', 'DESCRIÇÃO', 'Nº CARRO', 'MODELO', 'QTD', 'TIPO', 'MOTIVO', 'STATUS']; // CAMPO TIPO
    var linhas = registrosFiltrados.map(function(r) {
        return [r.codigo, r.descricao, r.numeroCarro, r.modelo.replace(/_/g, ' '), r.quantidade.toString(), r.tipo || 'N/A', r.motivo, r.status];
    });
