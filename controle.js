...
function registrarItem() {
    ...
    var transportador = document.getElementById('transportador').value.trim().toUpperCase();
    var notaFiscal = document.getElementById('notaFiscal').value.trim().toUpperCase();
...
function aplicarFiltros() {
    ...
    registrosFiltrados = registros.filter(function(r) {
        var matchBusca = !busca || r.codigo.toUpperCase().indexOf(busca) !== -1 || r.descricao.toUpperCase().indexOf(busca) !== -1 || (r.transportador && r.transportador.toUpperCase().indexOf(busca) !== -1) || (r.notaFiscal && r.notaFiscal.toUpperCase().indexOf(busca) !== -1);
...
function abrirEdicao(id) {
    ...
    var motivos = ['SUCATA', 'ERRO DE ESTOQUE', 'DIVERGÊNCIA DE ESTOQUE', 'PREVENTIVO'];
    ...
    modalHtml += '<div class="form-group"><label>ATRASO (DIAS)</label><input type="number" id="editAtraso" value="' + (registro.atraso || '') + '" min="0"></div>';
...
function exportarExcel() {
    ...
    var dados = registrosFiltrados.map(function(r) {
        return {
            ...
            'TIPO': r.tipo || 'N/A',
            'MOTIVO': r.motivo,
            'ATRASO': (r.atraso !== null && r.atraso !== undefined) ? r.atraso : 'N/A',
    ...
    ws['!cols'] = [
        ...
        { wch: 12 }, { wch: 20 }, { wch: 10 }, { wch: 20 }, { wch: 15 }, { wch: 18 }
    ];
...
function exportarPDF() {
    ...
    var linhas = registrosFiltrados.map(function(r) {
        return [
            ...
            r.quantidade.toString(), r.tipo || 'N/A', r.motivo, 
